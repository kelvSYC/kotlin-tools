...
// Port of nearest-rounding algorithm for DD using the same pattern as ddFloorOrCeil.
//
// When op(hi) == hi the high part is already an integer (or non-finite/zero).
//   Non-finite and zero: lo contributes nothing; return DD.of(op(hi)).
//   Finite integer: apply op to lo as well and re-normalise via DD.ofSum.
//     Adding 0.0 to op(lo) converts any -0.0 result to +0.0, which keeps the
//     low part of the result at exactly 0.0 when op(lo) == 0.
// When op(hi) != hi (hi is not an integer), lo has no effect; return DD.of(op(hi)).
private fun ddNearestRound(hi: Double, lo: Double, op: (Double) -> Double): DD {
    val y = op(hi)
    return if (y == hi) {
...
private object DdNearestRounding : FloatingPointNearestRounding<DD> {
    override fun DD.roundHalfUp(): DD = ddNearestRound(hi(), lo(), ::nearestRoundHalfUp)
    override fun DD.roundHalfDown(): DD = ddNearestRound(hi(), lo(), ::nearestRoundHalfDown)
    override fun DD.roundEven(): DD = ddNearestRound(hi(), lo(), ::nearestRoundEven)
}

/**
 * [FloatingPointNearestRounding] instance for Commons Numbers [DD].
 *
 * Implements the three nearest-integer rounding operations for DD values:
 ...
 * is independently rounded and the result is re-normalised via [DD.ofSum]. Otherwise the
 * result is the rounded high part with a zero low part.
 */
val FloatingPointNearestRounding.Companion.dd: FloatingPointNearestRounding<DD> get() = DdNearestRounding
