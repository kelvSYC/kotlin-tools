...
class FlatMultimapTest : FunSpec({

    context("construction") {
        test("emptyFlatMultimap produces an empty multimap") {
            val m = emptyFlatMultimap<String, Int>()
            m.isEmpty().shouldBeTrue()
            m.size shouldBe 0
        }

        test("flatMultimapOf with no arguments produces an empty multimap") {
            flatMultimapOf<String, Int>().isEmpty().shouldBeTrue()
        }

        test("flatMultimapOf with a single pair") {
            val m = flatMultimapOf("a" to 1)
            m.size shouldBe 1
            m["a"] shouldBe listOf(1)
        }

        test("toFlatMultimap from Iterable") {
            val m = listOf("a" to 1, "b" to 2, "a" to 3).toFlatMultimap()
            m.size shouldBe 3
            m["a"] shouldBe listOf(1, 3)
        }

        test("toFlatMultimap from Sequence") {
            val m = sequenceOf("a" to 1, "b" to 2, "a" to 3).toFlatMultimap()
            m.size shouldBe 3
            m["a"] shouldBe listOf(1, 3)
        }
    }
    ...
    context("properties") {
        val m = flatMultimapOf("a" to 1, "b" to 2, "a" to 3, "c" to 4)
    ...
    context("access") {
        val m = flatMultimapOf("a" to 1, "a" to 2, "b" to 3)
    ...
    context("null and empty helpers") {
        test("isNullOrEmpty returns true for null") {
            val m: FlatMultimap<String, Int>? = null
            m.isNullOrEmpty().shouldBeTrue()
        }

        test("isNullOrEmpty returns true for an empty multimap") {
            emptyFlatMultimap<String, Int>().isNullOrEmpty().shouldBeTrue()
        }

        test("orEmpty returns the same multimap when non-null") {
            val m: FlatMultimap<String, Int>? = flatMultimapOf("a" to 1)
    ...
    context("filter operations") {
        val m = flatMultimapOf("a" to 1, "a" to 2, "b" to 3, "c" to 4)
        ...
        test("filterValues omits keys that have no remaining values") {
            val result = flatMultimapOf("a" to 1, "b" to 2).filterValues { it > 5 }
    ...
    context("map operations") {
        ...
        test("mapKeys transforms each key") {
            val m = flatMultimapOf("a" to 1, "b" to 2)
        ...
        test("mapKeys concatenates value lists when keys collide, in entries order") {
            val m = flatMultimapOf("a" to 1, "b" to 2, "c" to 3)
    ...
    context("plus operator") {
        ...
        test("plus another multimap appends all entries of other in overall insertion order") {
            val other = flatMultimapOf("a" to 3, "c" to 4)
            val result = m + other
    ...
    context("equality and hashCode") {
        test("two multimaps with the same content are equal") {
            flatMultimapOf("a" to 1, "b" to 2) shouldBe flatMultimapOf("a" to 1, "b" to 2)
        }

        test("multimaps with different values are not equal") {
            flatMultimapOf("a" to 1) shouldNotBe flatMultimapOf("a" to 2)
        }

        test("multimaps with different duplicate counts are not equal") {
            flatMultimapOf("a" to 1, "a" to 1) shouldNotBe flatMultimapOf("a" to 1)
        }

        test("equal multimaps have the same hashCode") {
            val a = flatMultimapOf("a" to 1, "b" to 2)
    ...
    context("toString") {
        ...
        test("empty multimap toString is an empty list") {
            emptyFlatMultimap<String, Int>().toString() shouldBe "[]"
        }
    }

    context("conversion to ListMultimap") {
        test("toListMultimap groups entries by key, preserving per-key value order") {
            val m = flatMultimapOf("a" to 1, "b" to 2, "a" to 3)
            val result = m.toListMultimap()
            result.asMap shouldBe mapOf("a" to listOf(1, 3), "b" to listOf(2))
        }

        test("toListMultimap round-trip loses inter-key ordering") {
            val m = flatMultimapOf("a" to 1, "b" to 2, "a" to 3)
            val roundTrip = m.toListMultimap().toFlatMultimap()
            roundTrip.entries.toList() shouldBe listOf("a" to 1, "a" to 3, "b" to 2)
        }
    }
})
