...
class MutableListMultimapTest : FunSpec({
    ...
    context("put") {
        ...
        test("put preserves insertion order within a key") {
            val m = mutableListMultimapOf<String, Int>()
            m.put("a", 3)
            m.put("a", 1)
            m.put("a", 2)
    ...
    context("interop with ListMultimap read operations") {
        ...
        test("values returns all values including duplicates") {
            m.values.toList() shouldBe listOf(1, 2, 3)
        }

        test("entries returns all pairs") {
            m.entries.toList() shouldBe listOf("a" to 1, "a" to 2, "b" to 3)
        }
