...
/**
 * A [SetMultimap] whose keys are ordered by a [Comparator]. Iteration over [keys], [entries], and [values] always
 * follows comparator key order; values per key remain an unordered [Set].
 *
 * ### Range views
 *
 ...
 */
interface SortedSetMultimap<K, out V> : SetMultimap<K, V> {
    /**
    ...
    /**
     * Returns a read-only view of this multimap as a [SortedMap] mapping each key to its non-empty [Set] of values,
     * in comparator key order.
     */
    override val asMap: SortedMap<K, Set<@UnsafeVariance V>>
     ...
     * is `true`) [toKey].
     */
    fun headMultimap(toKey: K, inclusive: Boolean): SortedSetMultimap<K, V>
     ...
     */
    fun tailMultimap(fromKey: K, inclusive: Boolean): SortedSetMultimap<K, V>
     ...
     */
    fun subMultimap(fromKey: K, fromInclusive: Boolean, toKey: K, toInclusive: Boolean): SortedSetMultimap<K, V>

    /**
     * Returns a snapshot of all entries in this multimap in reverse comparator key order.
     */
    fun descendingMultimap(): SortedSetMultimap<K, V>
}
