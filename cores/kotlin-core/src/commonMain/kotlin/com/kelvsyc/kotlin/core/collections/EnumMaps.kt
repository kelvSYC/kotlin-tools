...
import com.kelvsyc.internal.kotlin.core.collections.ImmutableEnumMap
import kotlin.enums.enumEntries

/**
 * Returns an empty read-only [EnumMap] for enum type [K].
 */
inline fun <reified K : Enum<K>, V> emptyEnumMap(): EnumMap<K, V> = ImmutableEnumMap(ArrayEnumMap(enumEntries<K>()))
...
inline fun <reified K : Enum<K>, V> enumMapOf(vararg pairs: Pair<K, V>): EnumMap<K, V> =
    ImmutableEnumMap(ArrayEnumMap<K, V>(enumEntries<K>()).apply { putAll(pairs) })
...
inline fun <reified K : Enum<K>, V> buildEnumMap(builderAction: MutableEnumMap<K, V>.() -> Unit): EnumMap<K, V> =
    ImmutableEnumMap(ArrayEnumMap<K, V>(enumEntries<K>()).apply(builderAction))
...
inline fun <reified K : Enum<K>, V> Map<K, V>.toEnumMap(): EnumMap<K, V> =
    ImmutableEnumMap(ArrayEnumMap<K, V>(enumEntries<K>()).apply { putAll(this@toEnumMap) })
