...
import com.kelvsyc.internal.kotlin.core.collections.ImmutableEnumSet
import kotlin.enums.enumEntries

/**
 * Returns an empty read-only [EnumSet] for enum type [K].
 */
inline fun <reified K : Enum<K>> emptyEnumSet(): EnumSet<K> = ImmutableEnumSet(ArrayEnumSet(enumEntries<K>()))
...
inline fun <reified K : Enum<K>> enumSetOf(vararg elements: K): EnumSet<K> =
    ImmutableEnumSet(ArrayEnumSet<K>(enumEntries<K>()).apply { addAll(elements) })
...
inline fun <reified K : Enum<K>> buildEnumSet(builderAction: MutableEnumSet<K>.() -> Unit): EnumSet<K> =
    ImmutableEnumSet(ArrayEnumSet<K>(enumEntries<K>()).apply(builderAction))
...
inline fun <reified K : Enum<K>> Iterable<K>.toEnumSet(): EnumSet<K> =
    ImmutableEnumSet(ArrayEnumSet<K>(enumEntries<K>()).apply { addAll(this@toEnumSet) })
