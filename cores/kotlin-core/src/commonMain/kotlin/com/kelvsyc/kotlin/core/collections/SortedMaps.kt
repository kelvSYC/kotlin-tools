...
import com.kelvsyc.internal.kotlin.core.collections.ImmutableSortedMap
 ...
 */
fun <K, V> emptySortedMap(comparator: Comparator<in K>): SortedMap<K, V> = ImmutableSortedMap(TreeMap(comparator))
...
fun <K, V> sortedMapOf(comparator: Comparator<in K>, vararg pairs: Pair<K, V>): SortedMap<K, V> =
    ImmutableSortedMap(TreeMap<K, V>(comparator).also { pairs.forEach { (k, v) -> it[k] = v } })
...
fun <K, V> Iterable<Pair<K, V>>.toSortedMap(comparator: Comparator<in K>): SortedMap<K, V> =
    ImmutableSortedMap(TreeMap<K, V>(comparator).also { m -> forEach { (k, v) -> m[k] = v } })
...
): SortedMap<K, V> = ImmutableSortedMap(TreeMap<K, V>(comparator).apply(builderAction))
