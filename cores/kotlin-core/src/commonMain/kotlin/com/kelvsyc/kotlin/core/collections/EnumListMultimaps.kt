...
): EnumListMultimap<K, V> = ArrayEnumListMultimap<K, V>(enumEntries<K>()).apply(builderAction)
...
inline fun <reified K : Enum<K>, V> Iterable<Pair<K, V>>.toEnumListMultimap(): EnumListMultimap<K, V> =
    ArrayEnumListMultimap<K, V>(enumEntries<K>()).apply { putAll(this@toEnumListMultimap) }
