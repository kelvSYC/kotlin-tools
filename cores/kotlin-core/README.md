...
- **PriorityDeque** (`PriorityDeque<T>`) — double-ended priority queue interface extending `MutableCollection<T>`, backed by a min-max heap (Atkinson et al., 1986); supports O(1) `peekMin`/`peekMax` and O(log n) `pollMin`/`pollMax`, `add`/`remove`, `drainSorted`/`toSortedList`, bulk `removeAll`/`retainAll` with O(n) re-heapification, and O(n) bottom-up construction. **`PriorityDeque` does not extend `PriorityQueue`** and the two are not freely substitutable: unlike `Deque` vs. `Queue` (where the deque imposes zero overhead), a min-max heap sift must determine the current node's level at every step, adding a constant-factor cost even when only one end is used. Prefer `PriorityQueue` for single-ended algorithms (Dijkstra, Prim, A*). Factory functions: `priorityDequeOf(comparator)`, `priorityDequeOf(comparator, vararg elements)`, `minMaxPriorityDequeOf()`, `minMaxPriorityDequeOf(vararg elements)`, `Iterable<T>.toPriorityDeque(comparator)`, `Sequence<T>.toPriorityDeque(comparator)`
